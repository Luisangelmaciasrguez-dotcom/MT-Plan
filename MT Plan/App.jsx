      ...
      <div className="w-full" style={{ maxWidth: 360 }}>
        ...
        {!configured && (
          <p className="text-xs text-center mb-4 px-3 py-2 rounded-xl" style={{ color: C.personal, background: alpha(C.personal, 0.1) }}>
            Falta conectar Supabase. Sin esto la información NO se guarda. Revisa las variables de entorno en Vercel y vuelve a desplegar.
          </p>
        )}
        <input
          value={val}
          ...
          type="password"
          disabled={!configured}
          style={{ ...inputStyle, marginBottom: 12, opacity: configured ? 1 : 0.5 }}
          onKeyDown={(e) => { if (e.key === "Enter" && configured && val.trim()) onEnter(val.trim()); }}
        />
        <button onClick={() => configured && val.trim() && onEnter(val.trim())} disabled={!configured} className="w-full py-3.5 rounded-2xl font-semibold text-[15px]" style={{ background: configured ? C.personal : C.inkFaint, color: C.surface, opacity: configured ? 1 : 0.7 }}>
          Entrar
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [movimientos, setMovimientos] = useState([]); // {id, apartadoId, monto (+ hacia apartado / - de regreso), fecha}
  ...
  const currentDoc = () => ({ apartados, ingresos, egresos, inversionesLog, proyNuestros, proyRow, tareas, salud, movimientos });
  ...
  useEffect(() => {
    if (!spaceCode || !supabase || hydrating.current || !readyToSave.current) return;
    dirty.current = true;
    setSyncState("saving");
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apartados, ingresos, egresos, inversionesLog, proyNuestros, proyRow, tareas, salud, movimientos]);
  ...
  const apartadoBalance = (id) => {
    const movido = movimientos.filter((m) => m.apartadoId === id).reduce((s, m) => s + m.monto, 0);
    const gastado = egresos.filter((e) => e.apartadoId === id).reduce((s, e) => s + e.monto, 0);
    return movido - gastado;
  };

  const totalEnApartados = apartados.reduce((s, a) => s + apartadoBalance(a.id), 0);
  // Dinero libre: lo que hay en la cuenta y todavía no está repartido en apartados.
  const disponibleLibre = balanceGeneral - totalEnApartados;
  ...
  const addIngreso = () => {
    ...
    setIngresos([{ id: uid(), monto: Number(form.monto), categoria: form.categoria, fecha: form.fecha || todayISO() }, ...ingresos]);
    closeModal();
  };
  const addEgreso = () => {
    if (!form.monto || !form.categoria) return;
  ...
  // Mover dinero entre "disponible" y un apartado. dir: "in" (hacia apartado) | "out" (de regreso a disponible)
  const moverDinero = () => {
    const monto = Number(form.monto);
    if (!monto || !form.apartadoId) return;
    const signed = form.dir === "out" ? -monto : monto;
    setMovimientos([{ id: uid(), apartadoId: form.apartadoId, monto: signed, fecha: todayISO() }, ...movimientos]);
    closeModal();
  };
  const abrirMover = (apartadoId, dir) => { resetForm(); setForm({ apartadoId, dir }); setModal("mover"); };
      ...
      <div className="mx-auto relative" style={{ maxWidth: 448, minHeight: "100dvh", background: C.bg }}>
        ...
        <div className="px-5" style={{ paddingTop: "calc(20px + env(safe-area-inset-top))" }}>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setModal("config")} className="h-9 px-2.5 rounded-full flex items-center gap-1.5" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
                <Cloud size={15} color={syncState === "ready" ? C.good : syncState === "error" ? C.personal : C.inkFaint} />
                <Settings size={14} color={C.inkSoft} />
              </button>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <Card style={{ marginBottom: 0 }}>
                  ...
                  <p className="display text-lg font-bold" style={{ color: C.ink }}>{fmt(disponibleLibre)}</p>
                </Card>
                <Card style={{ marginBottom: 0 }}>
          ...
          {profile === "personal" && tab === "finanzas" && (
            <>
              ...
              {finSub === "apartados" && (
                <>
                  <Card>
                    <SectionLabel>Disponible para repartir</SectionLabel>
                    <p className="display text-2xl font-bold" style={{ color: disponibleLibre < 0 ? C.personal : C.ink }}>{fmt(disponibleLibre)}</p>
                    <p className="text-xs mt-1" style={{ color: C.inkSoft }}>Dinero en la cuenta que aún no está en ningún apartado.</p>
                  </Card>
                  {apartados.map((a) => (
                    <Card key={a.id}>
                      ...
                      <div className="flex gap-2 mt-3">
                        <button onClick={() => abrirMover(a.id, "in")} className="flex-1 py-2 rounded-xl text-[13px] font-semibold flex items-center justify-center gap-1.5" style={{ background: alpha(C.good, 0.12), color: C.good }}>
                          <Plus size={15} /> Agregar
                        </button>
                        <button onClick={() => abrirMover(a.id, "out")} className="flex-1 py-2 rounded-xl text-[13px] font-semibold flex items-center justify-center gap-1.5" style={{ background: C.soft, color: C.inkSoft, border: `1px solid ${C.line}` }}>
                          Retirar
                        </button>
                      </div>
                    </Card>
                  ...
                  <p className="text-xs text-center mt-1 px-4" style={{ color: C.inkFaint }}>
                    Registra tus ingresos normalmente; el dinero entra a "disponible". Cuando quieras, muévelo a un apartado con Agregar, o regrésalo con Retirar.
                  </p>
                </>
              )}
        ...
        {modal === "ingreso" && (
          <Modal title="Nuevo ingreso" onClose={closeModal}>
            ...
            <p className="text-[11px] mb-3" style={{ color: C.inkFaint }}>El dinero entra a "disponible". Luego lo repartes a tus apartados cuando quieras, desde Finanzas → Apartados.</p>
            <PrimaryButton onClick={addIngreso} color={C.good}>Guardar ingreso</PrimaryButton>
          </Modal>
        )}

        {modal === "mover" && (
          <Modal title={form.dir === "out" ? "Retirar de apartado" : "Agregar a apartado"} onClose={closeModal}>
            <Field label="Apartado">
              <select style={inputStyle} value={form.apartadoId || ""} onChange={(e) => setForm({ ...form, apartadoId: e.target.value })}>
                {apartados.map((a) => <option key={a.id} value={a.id}>{a.nombre}</option>)}
              </select>
            </Field>
            <Field label="Monto"><input type="number" style={inputStyle} placeholder="0" autoFocus onChange={(e) => setForm({ ...form, monto: e.target.value })} /></Field>
            <p className="text-xs mb-3" style={{ color: C.inkSoft }}>
              {form.dir === "out"
                ? `Regresa dinero de este apartado a disponible.`
                : `Disponible para repartir: ${fmt(disponibleLibre)}`}
            </p>
            <PrimaryButton onClick={moverDinero} color={form.dir === "out" ? C.inkSoft : C.good}>
              {form.dir === "out" ? "Retirar" : "Agregar al apartado"}
            </PrimaryButton>
          </Modal>
        )}
